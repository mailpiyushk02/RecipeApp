...
namespace RecipeApp_RecipeAPI.Controllers
{
    ...
    {
        private readonly IPermissionRepository _dbPermission;
        private readonly IMapper _mapper;
        protected APIResponse _response;
        public PermissionAPIController(IPermissionRepository dbPermission, IMapper mapper)
        {
            _dbPermission = dbPermission;
            _mapper = mapper;
        ...
        public async Task<ActionResult<APIResponse>> GetPermission()
        {
            try
            {
                IEnumerable<Permission> permissionList = await _dbPermission.GetAllAsync();
                _response.Result = _mapper.Map<List<PermissionDTO>>(permissionList);
        ...
        public async Task<ActionResult<APIResponse>> GetPermission(int id)
        {
            try
            {
                ...
                var permission = await _dbPermission.GetAsync(u => u.Id == id);
                if (permission == null)
                {
        ...
        public async Task<ActionResult<APIResponse>> DeletePermission(int id)
        {
            try
            {
                ...
                await _dbPermission.RemoveAsync(permission);
        ...
        //[HttpPut]
        //public async Task<ActionResult<APIResponse>> UpdatePermission(int id, [FromBody] PermissionUpdateDTO updateDTO)
        //{
        //    try
        //    {
        ...
        //            return BadRequest(_response);
        //        }
        //        var permission = _mapper.Map<Permission>(updateDTO);
        //        await _dbPermission.UpdateAsync(permission);
        //        _response.Result = _mapper.Map<PermissionUpdateDTO>(permission);
        //        _response.IsSuccess = true;
        //        return Ok(_response);
        //    }
        //    catch (Exception e)
        ...
        public async Task<ActionResult<APIResponse>> CreatePermission([FromBody] PermissionCreateDTO createDTO)
        {
            try
            {
                ...
                await _dbPermission.CreateAsync(permission);
                _response.StatusCode = HttpStatusCode.OK;
                _response.Result = _mapper.Map<PermissionDTO>(permission);
                _response.IsSuccess = true;
                return _response;
